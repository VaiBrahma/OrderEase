            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {orders.map(order => (
                    <div key={order._id} className="bg-white shadow-lg rounded-lg p-6">
                        ...
                        <div className="mb-2"><strong>Items:</strong> {JSON.stringify(order.items)}</div>
                        <div className="mb-2"><strong>Payment Method:</strong> {order.paymentMethod}</div>
                        <div className="mb-2"><strong>Total Amount:</strong> ${order.totalAmount}</div>
                        <div className="mb-2"><strong>Order Time:</strong> {new Date(order.orderTime).toLocaleString()}</div>
