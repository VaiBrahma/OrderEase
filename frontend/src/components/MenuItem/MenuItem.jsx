  ...
  const handleAdd = () => {
    setMenu((prevMenu) => {
      ...
      if (updatedMenu[item.name].quantity === 0)
        updatedMenu[item.name].quantity = value + 1;
  ...
  const handleRemove = () => {
    setMenu((prevMenu) => {
      ...
      if (updatedMenu[item.name] && updatedMenu[item.name].quantity > 0) {
        updatedMenu[item.name].quantity =
          updatedMenu[item.name].quantity - 1 / 2;
        if (updatedMenu[item.name].quantity === 0) {
  ...
  return (
    <div className={`${styles.card}`}>
      <div
        ...
        style={{
          background: `url(${item.image_src || "/images/defaultFoodIcon.jpeg"}) no-repeat center center/cover`,
        }}
      ></div>
      <div className={`${styles.right}`}>
        ...
        <div className={`${styles.price}`}>
          <span className="text-[#ffffff61]">Price:</span> ₹{item.price} /-
        </div>
        <div className={`${styles.buttons}`}>
          <button className={`${styles.remove}`}>
            <img src="/icons/remove.png" alt="-" onClick={handleRemove} />
          </button>
          <button className={`${styles.add}`}>
            <img src="/icons/add.png" alt="+" onClick={handleAdd} />
          </button>
        </div>
      </div>
    </div>
