...
const Menu = () => {
  ...
  const handleEdit = (item, newName, newPrice, newVegetarian) => {
    ...
    const edit = async () => {
      await axios
        .put(`/api/menu/${id}/updateItem`, updatedItemm)
        .then(() => {
          setMenu((prevMenu) =>
            prevMenu.map((item) =>
              item.id === itemId
                ? { ...item, name: newName, price: newPrice, vegetarian: bool }
                : item
            )
          );
        })
  ...
  const handleDelete = (item) => {
    ...
    const deletee = async () => {
      await axios
        .delete(`/api/menu/${id}/deleteItem`, { data: reqbody })
        .then(() => {
          setMenu((prevMenu) =>
            prevMenu.filter((itemm) => itemm.id !== item.id)
          );
        })
        .catch((err) => {
          console.log(err.message);
  ...
  return (
    <div>
      <button
        onClick={slide}
        className="z-10 bg-blue-600 rounded-full p-2 hover:bg-blue-700 text-white absolute m-4"
      >
        Add Item
      </button>
      <div>
      ...
      <div
        ref={formRef}
        className={`${styles.bg} border-[1px] border-[#d1d1d17c] mx-8 gap-4 rounded-3xl p-8 text-center flex flex-col bg-[#2c2c2cba] mb-[5rem] md:flex-row justify-center items-center`}
      >
